...
The investigation and restore process took over three days. The nearest backup turned out to be incomplete - the data at that point was already partially missing, suggesting the corruption had started earlier than we initially suspected. The provider's support team eventually restored from an older backup, which meant accepting the permanent loss of data from a specific window of time.
...
**Unreliable backups:** The nearest backup the provider had was already in an incomplete state, even before the incident we reported. This means there was a window of time where their backup system was silently failing to capture data correctly, with no indication or notification to us.
